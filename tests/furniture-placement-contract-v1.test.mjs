...
test("every source-accepted furniture identity has one explicit reviewed placement contract", () => {
  ...
  assert.equal(acceptedDefinitions.length, 40);
  assert.deepEqual(
  ...
  assert.equal(explicitDefinitions.filter((item) => item.placement.mount === "floor").length, 37);
  const expectedFloorPlacement = { mount: "floor", occupancy: "blocking", serviceAccess: "adjacent" };
  for (const id of ["banquette-section", "commercial-chair", "premium-chair", "host-stand-pro", "server-station-pro", "pos-terminal", "expo-pass-heated", "plancha-commercial", "convection-oven", "prep-table-refrigerated", "walkin-rack", "dry-storage-rack", "chemical-cabinet", "dish-machine-high-temp", "three-comp-sink", "wet-floor-station", "linen-storage", "water-station", "office-desk", "manager-console", "essential-cafe-two-top"]) {
