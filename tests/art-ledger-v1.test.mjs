...
test("the authoritative art ledger cannot hide missing, failed, local-only, or unbound art", () => {
  ...
  assert.equal(art.pendingBatches[0].id, "equipment-icons-dish-chef-004");
  ...
  assert.equal(equipmentSummary.present, 32);
  assert.equal(equipmentSummary.sourceAccepted, 32);
  assert.equal(equipmentSummary.remoteVerified, 24);
  assert.equal(equipmentSummary.productionComplete, 24);
  ...
  assert.equal(art.equipmentIcons.filter((entry) => entry.productionComplete).length, 24);
