...
test("the authoritative art ledger cannot hide missing, failed, local-only, or unbound art", () => {
  ...
  assert.equal(art.pendingBatches[0].id, "furniture-core-directional-003");
  ...
  assert.equal(furnitureSummary.present, 10);
  assert.equal(furnitureSummary.sourceAccepted, 10);
  assert.equal(furnitureSummary.remoteVerified, 7);
