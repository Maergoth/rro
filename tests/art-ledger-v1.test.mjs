...
test("the authoritative art ledger cannot hide missing, failed, local-only, or unbound art", () => {
  ...
  assert.equal(art.pendingBatches[0].id, "furniture-core-directional-002");
  ...
  assert.equal(furnitureSummary.present, 7);
  assert.equal(furnitureSummary.sourceAccepted, 7);
