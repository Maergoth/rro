...
test("the authoritative art ledger cannot hide missing, failed, local-only, or unbound art", () => {
  ...
  assert.deepEqual(art.pendingBatches.map((entry) => entry.id), ["furniture-core-directional-004"]);
  ...
  assert.equal(furnitureSummary.present, 13);
  assert.equal(furnitureSummary.sourceAccepted, 13);
  assert.equal(furnitureSummary.remoteVerified, 10);
