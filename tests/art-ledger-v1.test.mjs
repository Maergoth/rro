...
test("the authoritative art ledger cannot hide missing, failed, local-only, or unbound art", () => {
  ...
  assert.deepEqual(art.pendingBatches.map((entry) => entry.id), ["furniture-core-directional-005"]);
  ...
  assert.equal(furnitureSummary.present, 16);
  assert.equal(furnitureSummary.sourceAccepted, 16);
  assert.equal(furnitureSummary.remoteVerified, 13);
