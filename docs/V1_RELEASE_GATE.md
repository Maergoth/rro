...
| Modular layout | Pass (slice) | 24×16 cells; surfaces and room tags; one canonical authority per physical wall segment; walls/openings; floor, wall and ceiling objects; mount-aware drag/rotation; local staged preview with atomic commit/cancel; collision; path/egress validation; sale/repair; expansion that relocates perimeter openings and wall mounts; monotonic optimistic revisions; persistent audited undo/redo | Routed utilities/ventilation, code/clearance, native room-tag tool, richer collaboration and nav/flow heatmap |
