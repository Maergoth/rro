...
**Assessment baseline:** `1.0.0-alpha.1`  
**Protocol:** clean `rro.v1`  
**Assessment date:** 2026-07-31  
**Decision:** functional source alpha; not production 1.0; credible native foundation for a staffed V2 program
...
The migration changed the project from a browser systems prototype into a native, server-authoritative vertical-slice foundation. The source now demonstrates the defining loops: local identity and persistence, tactical world selection, exactly 20% NPC restaurant seeding, open shared shifts, seven role classes, continuous role queues, fallible multi-phase work, guest pressure, causal incidents/reviews, modular construction, seasonal data, randomized aptitude, character colors, and restaurant birth/death.
...
2. **Native alpha ready:** a real exported Windows client and standalone server have completed the recorded two-client acceptance session. This still requires a Godot/Windows runtime test outside the current environment.
...
| Area | Evidence in V1 |
|---|---|
| Engine boundary | Native Godot 4 project; no browser gameplay or baked restaurant image |
| Authority | HTTP/WebSocket service owns task phase/order/outcome, movement collision, money, XP, incidents, reviews, and layouts |
| Persistence | Fresh normalized SQLite line covering identity, world, construction, shifts, economy, evidence, and command idempotency |
...
| Failure | Risk choices, deadlines, off-role penalty, spills, sanitation, patience/satisfaction, evidence-based review effects |
| Builder | Cell surfaces/rooms, edge walls/openings, instance furniture/decor, drag, right-click rotation, collision, sale, expansion |
...
| Team handoff | Architecture/runbooks/GDD, deterministic generators, strict checks, integration tests, assignable backlog |

## Blocking gaps before native V1 promotion

- run the Godot project with the real 4.4.1 editor and resolve any engine-import/runtime-only defects;
...
- add inventory, tickets, recipes, food safety, utilities, equipment state, and deeper guest service causality;
...
The current tree may be called the **V1 source alpha foundation** after `npm run verify` passes. It must not be called a finished production 1.0 or AAA MMO until the corresponding gates above have evidence.
