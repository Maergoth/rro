...
func rebuild() -> void:
	...
	for surface in content.get("construction", {}).get("surfaces", []):
		...
		add_catalog_button(floors, "%s\n$%.2f/cell  ·  sanitation %+d  ·  ambience %+d" % [surface_label, float(surface.get("costCents", 0))/100.0, int(stats.get("sanitation", 0)), int(stats.get("ambience", 0))], func() -> void:
			selected_label.text = "%s floor brush" % surface_label
			tool_selected.emit("floor", surface_id)
		)
	...
	for wall in content.get("construction", {}).get("wallStyles", []):
		...
		add_catalog_button(walls, "%s wall\n$%.2f/edge  ·  snaps to the nearest cell edge" % [wall_label, float(wall.get("costCents", 0))/100.0], func() -> void:
			selected_label.text = "%s wall edge" % wall_label
			tool_selected.emit("wall", wall_id)
		)
	add_catalog_button(walls, "Service door opening\nUses painted plaster edge price", func() -> void:
		selected_label.text = "Service door · click a cell edge"
		tool_selected.emit("door", "painted-plaster")
	)
	add_catalog_button(walls, "Dining arch opening\nUses painted plaster edge price", func() -> void:
		selected_label.text = "Dining arch · click a cell edge"
		tool_selected.emit("arch", "painted-plaster")
	)
	var by_category := {}
	...
	for category in by_category.keys():
		...
		for item in by_category[category]:
			...
			add_catalog_button(catalog, "%s\n$%.2f  ·  %s  ·  %s" % [item_name, float(item.get("costCents", 0))/100.0, str(item.get("tier", item.get("style", "standard"))).capitalize(), modifier_text], func() -> void:
				selected_label.text = "%s · right-click to rotate" % item_name
				tool_selected.emit("object", item_id)
			)
...
func set_selected_object(object: Dictionary) -> void:
	selected_label.text = "Selected instance: %s · drag to move · right-click to rotate" % object.get("definitionId", "object")
